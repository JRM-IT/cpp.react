...
public:
    ...
    {
        impl::apply(AttachFunctor<D,TNode,TDeps...>{ node }, deps_);
    }

    template <typename D, typename TNode>
    void Detach(TNode& node) const
    {
		impl::apply(DetachFunctor<D,TNode,TDeps...>{ node }, deps_);
    }
    ...
    void AttachRec(const TFunctor& functor) const
    {
		...
		impl::apply(reinterpret_cast<const AttachFunctor<D,TNode,TDeps...>&>(functor), deps_);
    }

    template <typename D, typename TNode, typename TFunctor>
    void DetachRec(const TFunctor& functor) const
    {
		impl::apply(reinterpret_cast<const DetachFunctor<D,TNode,TDeps...>&>(functor), deps_);
    }
